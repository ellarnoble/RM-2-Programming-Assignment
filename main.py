...
for epoch in range(num_epochs):
    ...
    with torch.no_grad(): 
        for dev_embeddings, dev_labels in dev_loader_lr:
            ...
            correct += (predictions == dev_labels).sum().item()
    
    dev_accuracy = 100 * correct / total
    avg_loss = total_loss / len(train_loader_lr)
    logging.info(f"Epoch [{epoch+1}/{num_epochs}] | Loss: {avg_loss:.4f} | Dev Acc: {dev_accuracy:.2f}%")
...
logging.info("-" * 30)
logging.info(f"Best Dev Accuracy: {best_dev_acc:.2f}%")
logging.info("-" * 30)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for x_dev, mask_dev, labels_dev in trans_dev_loader:
            ...
            preds = (torch.sigmoid(logits_dev) > 0.5).long()
            correct += (preds == labels_dev).sum().item()
            total += labels_dev.size(0)
    
    acc = correct / total
    logging.info(f"Epoch [{epoch+1}/{num_epochs}] | Average Loss: {avg_loss:.4f} | Dev Accuracy: {acc:.4f}")

    """Implement early stopping logic """
    if acc > best_dev_acc_2:
        best_dev_acc_2 = acc
        patience_counter_2 = 0
    else:
...
logging.info(f"TRAINING COMPLETE")
logging.info("-" * 30)
logging.info(f"Best Dev Accuracy: {best_dev_acc_2 * 100:.2f}%")
logging.info("-" * 30)
